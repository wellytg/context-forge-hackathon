...
The `security-agent/` directory contains a complete, role-bounded agent
pipeline with 125 passing tests. If you want to run the same orchestration
against your own device roles and capability schema, read
[`AGENTS.md`](AGENTS.md) — it documents the exact build sequence, the
reusable step-by-step prompts, and the safety constraints that must hold
regardless of what schema you plug in.
